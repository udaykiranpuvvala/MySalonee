...
class ShopDetailsRepository {
    fun getShopDetailsResponse(jsonObjectParams: JsonObject): MutableLiveData<JsonElement?> {
        val loginResponse = MutableLiveData<JsonElement?>()

        RetrofitInstance.create().shopDetailsWithServices(jsonObjectParams).enqueue(object : Callback<JsonElement> {
