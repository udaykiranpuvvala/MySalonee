...
class PaymentGatewayRepository {
    fun getPaymentGatewayResponse(jsonObjectParams: JsonObject): MutableLiveData<JsonElement?> {
        val loginResponse = MutableLiveData<JsonElement?>()

        RetrofitInstance.create().paymentGateway(jsonObjectParams).enqueue(object : Callback<JsonElement> {
