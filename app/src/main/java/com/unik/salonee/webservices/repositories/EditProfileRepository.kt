...
class EditProfileRepository {
    fun getEditProfileResponse(jsonObjectParams: JsonObject): MutableLiveData<JsonElement?> {
        val loginResponse = MutableLiveData<JsonElement?>()

        RetrofitInstance.create().editProfile(jsonObjectParams).enqueue(object : Callback<JsonElement> {
