...
class BookingHistoryViewModel : ViewModel() {
    fun getBookingHistoryResponseViewModel(jsonObjectParams: JsonObject): MutableLiveData<JsonElement?> {
        return BookingHistoryRepository().getBookingHistoryResponse(jsonObjectParams)
    }
}