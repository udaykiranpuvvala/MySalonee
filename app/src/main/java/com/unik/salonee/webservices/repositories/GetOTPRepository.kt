...
class GetOTPRepository {
    fun getGetOTPResponse(jsonObjectParams: JsonObject): MutableLiveData<JsonElement?> {
        val loginResponse = MutableLiveData<JsonElement?>()

        RetrofitInstance.create().sendOtp(jsonObjectParams).enqueue(object : Callback<JsonElement> {
