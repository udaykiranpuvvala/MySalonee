...
class CitiesRepository {
    fun getCitiesResponse(jsonObjectParams: JsonObject): MutableLiveData<JsonElement?> {
        val loginResponse = MutableLiveData<JsonElement?>()

        RetrofitInstance.create().getCities(jsonObjectParams).enqueue(object : Callback<JsonElement> {
    ...
    fun getAllCitiesResponse(): MutableLiveData<JsonElement?> {
        val loginResponse = MutableLiveData<JsonElement?>()

        RetrofitInstance.create().getAllCities().enqueue(object : Callback<JsonElement> {
            override fun onFailure(call: Call<JsonElement>, t: Throwable) {
                Utility.showLog("Error", "Server Error : " + t.message)
                loginResponse.value = null
                Utility.hideLoadingDialog()
