...
class ServiceListRepository {
    fun getServiceListResponse(jsonObjectParams: JsonObject): MutableLiveData<JsonElement?> {
        val loginResponse = MutableLiveData<JsonElement?>()

        RetrofitInstance.create().serviceByCategoryId(jsonObjectParams).enqueue(object : Callback<JsonElement> {
