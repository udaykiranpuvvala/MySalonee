...
class LogInLogoutRepository {
    fun getLoginResponse(jsonObjectParams: JsonObject): MutableLiveData<JsonElement?> {
        val loginResponse = MutableLiveData<JsonElement?>()

        RetrofitInstance.create().loginUser(jsonObjectParams).enqueue(object : Callback<JsonElement> {
