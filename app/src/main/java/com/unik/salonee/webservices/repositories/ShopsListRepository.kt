...
class ShopsListRepository {
    fun getShopsResponse(jsonObjectParams: JsonObject): MutableLiveData<JsonElement?> {
        val loginResponse = MutableLiveData<JsonElement?>()

        RetrofitInstance.create().shopList(jsonObjectParams).enqueue(object : Callback<JsonElement> {
